...
function pickStatement(T, H) {
  if (T >= 27 && H >= 75) return "날씨가 더워요, 습도도 높아요. 주의";
  if (T >= 27 && H <= 40) return "덥고 건조해요. 급격한 수분 손실 주의";
  if (T <= 10 && H >= 70) return "매우 쌀쌀하고 습해요. 응결·곰팡이 주의";
  if (T <= 18 && H >= 70) return "쌀쌀하고 습해요. 환기/제습 필요";
  if (T <= 18 && H <= 49) return "쌀쌀하고 건조해요. 보온/가습 고려";
  if (H >= 70)           return "습도가 매우 높아요. 제습/환기 필수";
  if (H <= 49)           return "습도가 매우 낮아요. 가습/관수 필요";
  if (T >= 27)           return "날씨가 더워요";
  if (T > 18 && T < 27 && H > 49 && H < 70) return "맑네요. 상태가 양호합니다.";
  return "상태 판단 불가";
}
